...
namespace Loria.Core.Web
{
    public class HttpModule : IModule, IListener
    {
        ...
        public string Listen() => string.Empty;

        public string HandleRequest(HttpListenerRequest request)
        {
            var url = request.RawUrl.Substring(1);
            var message = string.Join(" ", url.Split('/'));

            Configuration.Hub(message);
            return "OK";
        }
    }
